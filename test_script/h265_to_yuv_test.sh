...
input_folder="../h265_files"
output_folder="../test_output/h265_to_yuv"
result_folder="../test_result/h265_to_yuv"
iterations=20

mkdir -p "$output_folder"
mkdir -p "$result_folder"

echo "Starting batch decoding tests..."
echo "======================================"


for input_file in "$input_folder"/*.265; do
    ...
    output_yuv="$output_folder/${filename_noext}.yuv"
    ...
    for i in $(seq 1 $iterations); do
        ...
        output=$(sudo ../build/xcoder -c 0 -i "$input_file" -o "$output_yuv" -m h2y 2>&1)
        end_time=$(date +%s.%N)

        total_bytes=$(echo "$output" | grep "Total bytes" | tail -n 1 | awk '{print $8}')
