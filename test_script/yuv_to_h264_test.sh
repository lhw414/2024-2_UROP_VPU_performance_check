...
output_folder="../test_output/yuv_to_h264"
result_folder="../test_result/yuv_to_h264"
iterations=20
...
for input_file in "$input_folder"/*.yuv; do
    ...
    output_yuv="$output_folder/${filename_noext}.264"
    ...
    for i in $(seq 1 $iterations); do
        ...
        if [ -n "$resolution" ]; then
            output=$(sudo ../build/xcoder -c 0 -i "$input_file" -o "$output_yuv" -m y2a -s "$resolution" 2>&1)
        else
            output=$(sudo ../build/xcoder -c 0 -i "$input_file" -o "$output_yuv" -m y2a 2>&1)
        fi
        end_time=$(date +%s.%N)
